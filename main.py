...
@app.get("/future-scope", response_class=HTMLResponse)
def page_future_scope():
    """Serves the standalone conceptual mockup for Future Scope Occasion-Aware Gifting."""
    with open("future_scope_occasion_gifting.html", "r", encoding="utf-8") as f:
        html_content = f.read()
...
def page_payment_link(request: Request, link_id: str):
    ...
    return templates.TemplateResponse("pay.html", {
        ...
        "original_price_paise": original_amount_paise,
...
def list_campaigns_endpoint(merchant_id: Optional[str] = None):
    """GET /api/campaigns — Retrieve campaigns for a merchant store or all merchants."""
    campaigns = get_campaigns_by_merchant(merchant_id) if merchant_id else get_active_campaigns()
